...
int cgl_read_section_header(const char *name, FILE *fp)
{
	...
	if (nread < CGL_SHDR_SIZE) {
		printf("cgl %s header incomplete", name);
		return -EBADSHDR;
	} else if (memcmp(hdr, name, CGL_SHDR_SIZE) != 0) {
		printf("cgl %s header corrupted", name);
		return -EBADSOIN;
	}
	return 0;
}
 ...
 * On error printf(...) is called and non-zero error code is returned.
...
int cgl_read_size(struct cgl *cgl, FILE *fp)
{
	...
	if (err) {
		printf("cgl SIZE section corrupted (incomplete)");
		return -EBADSIZE;
	}
	cgl->width  = dims[0];
	cgl->height = dims[1];
...
int cgl_read_soin(struct cgl *cgl, uint8_t *nums, FILE *fp)
{
	...
	if (nread < nblocks) {
		printf("cgl SOIN section corrupted (incomplete)");
		return -EBADSOIN;
	}
	cgl->ntiles = 0;
	for (size_t j = 0; j < cgl->height; ++j)
...
int cgl_read_magic(struct cgl *cgl, FILE *fp)
{
	...
	if (nread < CGL_MAGIC_SIZE) {
		...
		printf("cgl corrupted (error reading magic/header");
		return -EBADSOBS;
...
int read_block(struct tile *tiles, size_t num, int x, int y, FILE* fp)
{
	...
	for (size_t k = 0; k < num; ++k) {
		...
		if (nread < SOBS_TILE_SIZE) {
			printf("cgl SOBS section corrupted "
					"(incomplete)");
			return -EBADSOBS;
		}
...
#define END_CGL_READ_X(what, hdr, obj, howmany) \
	...
	return 0;                                                           \
error:                                                                      \
	printf("cgl " #hdr " section corrupted (incomplete)");        \
	return -EBAD##hdr;                                                  \
}
